const template=`
    <el-tabs type="card" v-model="tabsValue" v-if="tabs" @tab-change="tabChange">
        ...
        <el-tab-pane v-for="(tab,key) in tabList" :name="key" :label="__(tab)"></el-tab-pane>
    </el-tabs>
    <div class="toolbar">
        <el-row>
            <el-col :xs="24" :sm="24" :md="12" :style="(pageFont=='small')?'margin-bottom:10px;':'margin-bottom:20px;'">
                <div class="left">
                    <el-button-group :size="pageFont">
                        <template v-for="tool in table_.toolbar">
                            ...
                            <el-button v-if="auth.recyclebin && tool=='recyclebin'" @click.stop="recyclebin" type="success"><i class="fa fa-recycle"></i>&nbsp;${__('回收站')}</el-button>    
    ...
    <el-card class="filter" shadow="hover" v-show="commonSearch && table_.searchFormVisible">
        <el-form label-width="100" label-position="left">
            <el-row :gutter="20">
                <template v-for="(column,key) in table_.columns">
    ...
    <el-table
            ...
            <el-table-column
                    ...
                    <template #default="scope">
                        <slot name="expand" :rows="scope.row"></slot>
                    </template>
            </el-table-column>
            <template v-for="(column,key) in table_.columns">
                <el-table-column :align="column.align?column.align:'center'" v-if="column.checkbox" type="selection" :selectable="column.selectable"></el-table-column>
                <el-table-column :align="column.align?column.align:'center'" v-if="column.treeExpand" width="80">
                    <template #header >
                         <el-button @click="expandAllTree()" :class="['el-button-icon',pageFont]" type="success" size="small"><i :class="treeExpandAll_?'fa fa-chevron-up':'fa fa-chevron-down'"></i></el-button>
                    </template>
                    <template #default="scope">
                        <el-button @click="expandTree(scope.row.id)" :class="['el-button-icon',pageFont]" :type="scope.row.haschild?'success':'info'" size="small"><i class="fa fa-sitemap"></i></el-button>
                    </template>
                </el-table-column>
                <el-table-column
                    ...
                    :prop="column.field">
                    <template #header>
                    <slot name="header" :field="column.field">{{column.title}}</slot>
                    </template>
                    <template #default="scope" v-if="column.field!='operate'">
                        <template v-if="scope.row._formatter">
                            ...
                            <template v-if="scope.row._formatter[column.field]._name=='link'">
                                ...
                                <template #append>
                                    <a target="_blank" :href="scope.row._formatter[column.field].value"><i class="fa fa-link"></i></a>
                                </template>
                                </el-input>
                            </template>
                    ...
                    <template #default="scope" v-if="column.field=='operate' && column.action">
                         <div :class="'operate-box-'+column.direction">
                             <template v-for="(rx,action) in column.action"> 
                                    <el-tooltip
                                    ...
                                    <el-tooltip
                                            v-if="((typeof rx=='boolean' && rx) || (typeof rx=='function' && rx(scope.row))) && action=='expand'"
                                            effect="dark"
                                            :hide-after="0"
                                            content="${__('展开')}"
                                            placement="top">
                                        <el-button @click="rowsExpand(scope.row)" type="info" size="small" :class="['el-button-icon',pageFont]"><i :class="scope.row.isExpand?'fa fa-chevron-down':'fa fa-chevron-right'"></i></el-button>
                                    </el-tooltip>
        ...
        <el-form labelWidth="120" labelPostion="top">
               <el-form-item label="列表">
                    <el-checkbox-group v-model="download.field">
                        ...
                        <el-checkbox
                            checked
                            v-if="column.field && column.field!='operate' && column.visible!='none'"
                            :label="column.field">
                            {{column.title}}
                        </el-checkbox>
                        </template>
