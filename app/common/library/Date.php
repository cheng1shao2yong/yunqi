...
class Date
{
     ...
     */
    public static function span(int $remote, int $local = null, string $output = 'years,months,weeks,days,hours,minutes,seconds'):mixed
    {
        // Normalize output
     ...
     */
    public static function human(int $remote, mixed $local = null):string
    {
    ...
    public static function unixtime(string $type = 'day', int $offset = 0, string $position = 'begin', int $year = null, int $month = null, int $day = null, int $hour = null, int $minute = null):int
    {
        ...
        $month = is_null($month) ? intval(date('m')) : $month;
        $day = is_null($day) ? intval(date('d')) : $day;
        $hour = is_null($hour) ? intval(date('H')) : $hour;
        $minute = is_null($minute) ? intval(date('i')) : $minute;
        ...
        switch ($type) {
            ...
            case 'month':
                ...
                $time = $position ? $_timestamp : mktime(23, 59, 59, $month + $offset, self::days_in_month(intval(date("m", $_timestamp)), intval(date("Y", $_timestamp))), $year);
                break;
            case 'quarter':
     ...
     */
    public static function days_in_month(int $month, int $year):mixed
    {
